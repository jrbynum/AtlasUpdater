...
namespace AtlasUpdater.Interfaces
{
	abstract class BackupInterface
	{
		...
		public BackupInterface(AtlasUpdater Parent)
		{
			this._Parent = Parent;
		}
	...
	class BackupInterfaceWindows : BackupInterface
	{
		...
		public override bool BackupServer(SettingsLoader.ServerChild ServerData)
		{
			...
			if( _Parent.ATLASConfiguration.Messages.ServerBackupBroadcast.Length >= 1 )
			{
				using( var RCONClient = new ATLASRCON("127.0.0.1", ServerData.RCONPort) )
				{
					try
					{
						RCONClient.Authenticate(ServerData.ServerAdminPassword);
						RCONClient.ExecuteCommand(string.Format("serverchat {0}", _Parent.ATLASConfiguration.Messages.ServerBackupBroadcast));
		...
		public override bool CleanBackups(SettingsLoader.ServerChild ServerData)
		{
			try
			{
				...
				if( BackupFiles.Length > _Parent.ATLASConfiguration.Backup.NumberOfBackupsToKeepPerServer )
				{
					...
					var KeepItems = Dict.OrderByDescending( x => x.Value ).Take( _Parent.ATLASConfiguration.Backup.NumberOfBackupsToKeepPerServer ).ToDictionary( x => x.Key );
					foreach( var tFile in BackupFiles )
					{
	...
	{
		public BackupInterfaceUnix(AtlasUpdater parent) : base(parent) {}
		public override bool BackupServer(SettingsLoader.ServerChild ServerData)
		{
