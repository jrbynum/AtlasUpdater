...
namespace AtlasUpdater.Interfaces
{
	...
	{
		protected AtlasUpdater _Parent;
		protected Dictionary<Process, SettingsLoader.ServerChild> _ProcessDict;
		public ServerInterface(AtlasUpdater Parent)
		{
	...
	class ServerInterfaceWindows : ServerInterface
	{
		...
		public ServerInterfaceWindows(AtlasUpdater parent) : base(parent) {}
		...
		public override int StartServer(SettingsLoader.ServerChild ServerData)
		{
			...
			if( !_Parent.ATLASConfiguration.UseServerNameInINIFile ) QueryString.Append("?ServerName="+ServerData.GameServerName);
			...
			try
			{
				...
				// Set Window Title
				Task tResult = this._UpdateWindow(Proc, string.Format("ATLAS: {0} (Managed by AtlasUpdater)", ServerData.GameServerName));
		...
		private void _ProcessExited(object sender, EventArgs e)
		{
			...
			foreach( var Server in _Parent.Servers )
			{
				...
				_Parent.Log.ConsolePrint(LogLevel.Debug, "Server '{0}' completed shutdown, mATLASing as ready for update", Server.ServerData.GameServerName);
				Server.ProcessID = 0;
				break;
			}
	...
	{
		public ServerInterfaceUnix(AtlasUpdater parent) : base(parent) {}
		public override int StartServer(SettingsLoader.ServerChild ServerData)
		{
